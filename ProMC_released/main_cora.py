...
def meta_test(model, test_features, test_labels, idx_test,adj,args):
    ...
    if args.baseline:
        with torch.no_grad():
            output,_,_,_,_= model(test_features,adj, test_labels[idx_test], idx_test,args,False)
...
def main(args):
    ...
    for i in range(int(len(combination))):
        ...
        optimizer = torch.optim.Adam(model.parameters(), lr=args.lr,weight_decay=args.weight_decay*5000)###/5 3000
        model.train()
        optimizer.zero_grad()
        lossf=torch.nn.CrossEntropyLoss()
    ...
    print(torch.tensor(mean_accuracy_meta_test).numpy().mean())
...
if __name__ == '__main__':
    ...
    parser.add_argument('--baseline', type=bool, default=False, help='as baseline model')
